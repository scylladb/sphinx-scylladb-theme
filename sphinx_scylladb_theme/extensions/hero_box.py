...
class HeroBox(Directive):
    ...
    def run(self):
        ...
        if button_icon_position == "right":
            button_content = f"{button_content}<i class='icon icon--right {button_icon}' aria-hidden='true'></i>"
        else:
            button_content = f"<i class='icon icon--left {button_icon} ' aria-hidden='true'></i>{button_content}"
