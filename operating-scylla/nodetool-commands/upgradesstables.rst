...
``<options>`` are the `Nodetool Generic Options </nodetool/#nodetool-generic-options>`_
...
* ``--`` Separator which can be used to seperate an option from an argument. For example, if you are using the ``-a`` flag, use the separator between the flag and the ``keyspace-name``
...
`Encryption at Rest </operating-scylla/security/encryption-at-rest>`_
