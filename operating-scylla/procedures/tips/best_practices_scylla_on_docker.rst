...
++++++++++++++++++++++++++++++++++++
The Docker image also has Scylla’s utilities installed. `Nodetool <http://www.scylladb.com/doc/nodetool/>`_ is a command line tool for querying and managing a ScyllaDB cluster. The simplest ``nodetool`` command is ``nodetool status``, which displays information about the cluster state:
...
1. On the host, create and edit for example ``master_scylla.yaml``. Uncomment and change the "cluster_name" parameter.
