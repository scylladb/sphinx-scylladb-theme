...
Specifies maximum number of table names printed for a keyspace.
Use ``-1`` for no limit.
...
.. code-block:: none
   ...
   sctool backup -c prod-cluster -s '2019-12-09T15:16:05Z00:00’ -i 24h -L 's3:my-backups’
...
.. code-block:: none
   ...
   [--max-date <date>] [--min-date <date>] [--show-tables <number of tables>][global flags]
...
.. code-block:: none
   ...
   sctool backup list --cluster test-cluster --show-tables 5 
   Snapshots:
...
.. code-block:: none
   ...
   sctool cluster add --host 34.203.122.52 --auth-token "6Es3dm24U72NzAu9ANWmU3C4ALyVZhwwPZZPWtK10eYGHJ24wMoh9SQxRZEluWMc0qDrsWCCshvfhk9uewOimQS2x5yNTYUEoIkO1VpSmTFu5fsFyoDgEkmNrCJpXtfM"  --name test-cluster 
   ...
   |  |     $ export SCYLLA_MANAGER_CLUSTER=test-cluster
   || |/    
   || ||    Now run:
   |\_/|    $ sctool status -c test-cluster
   \___/    $ sctool task list -c test-cluster

Example (IPv6):

.. code-block:: none

   sctool cluster add --host 2a05:d018:223:f00:971d:14af:6418:fe2d --auth-token       "6Es3dm24U72NzAu9ANWmU3C4ALyVZhwwPZZPWtK10eYGHJ24wMoh9SQxRZEluWMc0qDrsWCCshvfhk9uewOimQS2x5yNTYUEoIkO1VpSmTFu5fsFyoDgEkmNrCJpXtfM"  --name test-cluster
...
.. code-block:: none
   ...
   sctool cluster delete -c test-cluster
...
.. code-block:: none
   ...
   ├──────────────────────────────────────┼─────────────────────┼────────────────┤
   │ db7faf98-7cc4-4a08-b707-2bc59d65551e │ test-cluster        │ scylla-manager │
   ╰──────────────────────────────────────┴─────────────────────┴────────────────╯
...
.. code-block:: none
   ...
   sctool cluster update --test-cluster cluster --name prod-cluster 
...
This means that if the glob patterns matches nothing, a repair will still run.
...
It disables the built-in Scylla mechanism for repair and instead, uses only the IP or hostname you set here.
...
By listing the host IP address, you run a repair only on the the specified host.
...
The SSL status can be `ON` if SSL is enabled or `OFF` if it is disabled.
If you are using an SSH to connect to the clusters, the SSL status will be `OFF`.
The REST API status can be `UP` or `DOWN`.
If it is `DOWN`, it means that the connection to the nodes over the REST API has been lost.
...
The `SCYLLA_MANAGER_CLUSTER` must also be included.
...
.. code-block:: none
   ...
   sctool task progress repair/dff91fd1-f430-4f98-8932-373644fe647e -c test-cluster --details
   Status:         RUNNING
   Start time:     17 Apr 19 12:55:57 UTC
   Duration:       3m0s
   Progress:       1.91%
...
.. code-block:: none
   ...
   sctool task progress --cluster test-cluster
   backup/f0642e3e-e1cb-44ba-8447-f8d43672bcfd
   Arguments: -L s3:backups
   Status: DONE
   Start time: 16 Dec 19 16:46:08 CET
   End time: 16 Dec 19 16:46:13 CET
   Duration: 5s
   Progress: 100%
   Snapshot Tag: sm_20191216154608UTC
   Datacenters:

   dc1
   dc2
   ╭────────────────┬──────────┬────────┬───────────┬──────────┬──────────────┬────────╮
   │ host           │ progress │ size   │ processed │ uploaded │ deduplicated │ failed │
   ├────────────────┼──────────┼────────┼───────────┼──────────┼──────────────┼────────┤
   │ 192.168.100.11 │ 100%     │ 4.8MiB │ 4.8MiB    │ 0B       │ 4.8MiB       │ 0B     │
   │ 192.168.100.12 │ 100%     │ 4.8MiB │ 4.8MiB    │ 0B       │ 4.8MiB       │ 0B     │
   │ 192.168.100.13 │ 100%     │ 4.8MiB │ 4.8MiB    │ 0B       │ 4.8MiB       │ 0B     │
   │ 192.168.100.21 │ 100%     │ 4.8MiB │ 4.8MiB    │ 0B       │ 4.8MiB       │ 0B     │
   │ 192.168.100.22 │ 100%     │ 4.8MiB │ 4.8MiB    │ 0B       │ 4.8MiB       │ 0B     │
   │ 192.168.100.23 │ 100%     │ 4.8MiB │ 4.8MiB    │ 0B       │ 4.8MiB       │ 0B     │
   ╰────────────────┴──────────┴────────┴───────────┴──────────┴──────────────┴────────╯
...
.. code-block:: none 
   ...
   sctool task stop --cluster test-cluster repair/143d160f-e53c-4890-a9e7-149561376cfd 
...
.. code-block:: none
   ...
   sctool task update -c test-cluster repair/4d79ee63-7721-4105-8c6a-5b98c65c3e21 --enabled false
...
.. code-block:: none 
   ...
   sctool task update -c test-cluster repair/143d160f-e53c-4890-a9e7-149561376cfd -s now+3h --interval 2d
...
.. code-block:: none
   ...
   sctool version
   Client version: 2.0-0.20191210.95c4bffd
   Server version: 2.0-0.20191210.95c4bffd
