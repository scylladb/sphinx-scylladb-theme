...
Is supported in Scylla version 2.2 and later and should be used instead of USERS.
...
``USER``. For backward compatibility, this syntax has been preserved. From Scylla 2.2 and onward it is recommended to use `roles <db-roles>`_.
...
Attempting to ``GRANT`` a permission on resource to which it cannot be applied results in an error response. The following illustrates which
...
This gives any user with the ``data_reader`` permission to execute ``SELECT`` statements on any table across all
keyspaces::

    GRANT MODIFY ON KEYSPACE keyspace1 TO data_writer;

This give any user with the ``data_writer`` permission to perform ``UPDATE``, ``INSERT``, ``UPDATE``, ``DELETE``
and ``TRUNCATE`` queries on all tables in the ``keyspace1`` keyspace::

    GRANT DROP ON keyspace1.table1 TO schema_owner;

This gives any user with the ``schema_owner`` permissions to ``DROP`` ``keyspace1.table1``
...
Revoking a permission from a user uses the ``REVOKE PERMISSION`` statement:
...
* `Role Based Access Control (RBAC) </operating-scylla/security/rbac_usecase/>`_
