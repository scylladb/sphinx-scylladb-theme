...
Any unfiltered in a partition is either a row or a ``range_tombstone_marker``. We will examine both of them below.
...
1. `A concise but good overview of main features of the SSTables 3.0 format including some measurements of disk 
   space savings compared to 2.x.`_

2. A very `detailed article explaining SSTables 3.0 data file format`_ with lots of references to the relevant code 
   parts. This article has a few inaccuracies like complex cells format but otherwise is a great read with a lot 
   of useful details. 
 
3. `This article`_ covers tombstones in SSTables 3.0 in a great detail.

4. An `engineering description`_ of changes in SSTables data format in Cassandra 3.0.
   Explains well a variety of higher-level concepts and sheds light on some lower-level implementation details.

5. Gives a `brief overview`_ of SSTables 3.0 data format and provides some interesting exampes of dumps of data 
   files. 

6. There is hardly any documentation that would be more accurate and up-to-date than the actual `source code`_
  
.. _`A concise but good overview of main features of the SSTables 3.0 format including some measurements of disk 
   space savings compared to 2.x.`: https://www.datastax.com/2015/12/storage-engine-30

.. _`detailed article explaining SSTables 3.0 data file format`: http://thelastpickle.com/blog/2016/03/04/introductiont-to-the-apache-cassandra-3-storage-engine.html

.. _`This article`: http://thelastpickle.com/blog/2016/07/27/about-deletes-and-tombstones.html

.. _`engineering description`: https://apache.googlesource.com/cassandra/+show/cassandra-3.0.0-rc2/guide_8099.md

.. _`brief overview`: https://www.esentri.com/blog/2017/05/08/sorted-string-table-sstable-in-apache-cassandra/

.. _`source code`: https://github.com/apache/cassandra/tree/cassandra-3.0
