...
@pytest.mark.parametrize(
    "role_name, input_text, expected_output, glossary, is_error", tooltip_data
)
def test_include_tooltip_role(
    role_name, input_text, expected_output, glossary, is_error
):
    glossary = glossary or {}
    mock_inliner.document.settings.env.get_domain = Mock(
        return_value=Mock(_terms=glossary)
    )
    ...
    result, messages = include_tooltip_role(
    ...
    assert (
        bs(output_html, "html.parser").prettify()
        == bs(expected_output, "html.parser").prettify()
    )
