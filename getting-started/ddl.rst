...
which is the replication strategy used by the keyspace. It is generally encouraged to use
...
In all cases for creating keyspaces and tables, if you are using `Reserved Keywords </getting-started/reserved_keywords>`_, enclose them in single or double quotes.
...
strategy is used. By default, Scylla support the following ``'class'``:

- ``'SimpleStrategy'``: defines a replication factor for the whole cluster. The only sub-options
  supported is ``'replication_factor'`` to define that replication factor and is mandatory.
- ``'NetworkTopologyStrategy'``: A replication strategy that lets you set the replication factor independently for
...
.. _Auto-expand_replication_factor:
...
Note than when adding new data centers, you need to set the Replication Factor using the auto-expansion option a second time, to include the added Data Centers. 
...
The ``USE`` statement allows you to change the *current* keyspace (for the *connection* on which it is executed). A number
...
of data modeling in Scylla, and it largely impacts which queries can be performed, and how efficient they are.
...
``b`` column (the order they are displayed above). So where the partition keys of the table lets you group rows on the
...
..
   .. warning:: Since Apache Cassandra 3.0, compact tables have the exact same layout internally than non compact ones (for the
...
default, that ordering is based on natural order of the clustering order, but the ``CLUSTERING ORDER`` lets you
...
+--------------------------------+----------+-------------+-----------------------------------------------------------+
| ``read_repair_chance``         | *simple* | 0           | The probability that extra nodes are queried (e.g.        |
|                                |          |             | more nodes than required by the consistency level) for    |
|                                |          |             | the purpose of read repairs.                              |
+--------------------------------+----------+-------------+-----------------------------------------------------------+
| ``dclocal_read_repair_chance`` | *simple* | 0.1         | The probability that extra nodeas are queried (e.g.       |
|                                |          |             | more nodes than required by the consistency level)        |
|                                |          |             | belonging to the same data center as the read             |
...
|                                |          |             | bloom filters. Sstable bloom filters will be sized to     |
|                                |          |             | provide the provided probability (thus lowering this value|
...
+--------------------------------+----------+-------------+-----------------------------------------------------------+
|``cdc``                         | *map*    | *see below* | :ref:`CDC options <cdc-options>`.                         |
+--------------------------------+----------+-------------+-----------------------------------------------------------+
...
the strategy chosen (see the section corresponding to your strategy for details: `STCS </getting-started/compaction/#stcs-options>`_, `LCS </getting-started/compaction/#lcs-options>`_, `ICS </getting-started/compaction/#ics-options>`_ and `TWCS </getting-started/compaction/#twcs-options>`_). DTCS is not recommended and TWCS should be used instead.
...
========================= =============== =============================================================================
 ``class``                 LZ4Compressor   The compression algorithm to use. Default compressors are: LZ4Compressor,
...
To use this feature you must enable the `experimental tag </operating-scylla/scylla-yaml/#enabling-experimental-features>`_ in the scylla.yaml.
...
+---------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------+
| ``ttl``                   | 86400 seconds   | Time after which data stored in CDC will be removed and won’t be accessible to the client any more.                    |
|                           | 24 hours        |                                                                                                                        |
+---------------------------+-----------------+------------------------------------------------------------------------------------------------------------------------+
...
.. note:: When the ``key_provider`` is ``LocalFileSystemKeyProviderFactory``, you must indicate where the key resides using the ``secret_key_file: <path>`` parameter. Refer to `Encryption at Rest </operating-scylla/security/encryption-at-rest>`_ for details. 
...
- Change or add any of the `Encryption options`_ above.
