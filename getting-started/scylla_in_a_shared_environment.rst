...
for shared environments. For some production environment these settings
may be preferred as well.

Note that a docker image is a viable and even simpler option - `Scylla
on dockerhub <https://hub.docker.com/r/scylladb/scylla/>`__

Memory
------

The most important resource that Scylla consumes is memory. By default,
...
On Ubuntu, edit ``/etc/default/scylla-server``, and add ``--memory 2G``
...
-  ``--overprovisioned`` tells Scylla that the machine it is running on
   is used by other processes; so Scylla will not pin it threads or
