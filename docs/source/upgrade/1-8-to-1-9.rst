...
.. code-block::
    ...
    dependencies = [
        ...
        "sphinx>=9.0",
        "sphinx-multiversion-scylla>=0.3.6",
...
#. Add ``permissions: contents: read`` at the top level of ``docs-pr.yaml``:
...
#. In ``docs-pages.yaml``, update the ``release`` job permissions to use ``contents: write``:
