...
This document gives a high level overview of Compaction focusing on what compaction is, and how it works. There are separate documents which cover the `CQL syntax </getting-started/compaction/>`_ for setting a compaction strategy and `Compaction Strategy Matrix </architecture/compaction/compaction-strategies/>`_ which covers how to decide which strategy works best.
...
.. image:: compaction-datawrites.png
...
Scylla has tools you can use to see the status of your compactions. These include nodetool (`compactionhistory </operating-scylla/nodetool-commands/compactionhistory/>`_  and `compactionstats </operating-scylla/nodetool-commands/compactionstats/>`_)  and the Grafana dashboards which are part of the `Scylla Monitoring Stack </operating-scylla/monitoring/monitoring_stack/>`_ which display the compaction statistics on a per cluster and per node basis.  Compaction errors can be seen in the `logs </operating-scylla/manager/1.3/configuration-file/#logging-settings>`_. 

Compaction strategy
-------------------

A compaction strategy is what determines which of the SSTables will be compacted, and when. The following compaction strategies are available and are described in greater detail below. For a matrix which compares each strategy to its workload, refer to `Compaction Strategy Matrix </architecture/compaction/compaction-strategies/>`_
...
Compaction strategies are set as part of the ``CREATE`` or ``ALTER`` statement when creating or altering tables. Refer to the `CQL syntax </getting-started/compaction/>`_ for details. 
...
An SSTable is added to an existing bucket if size of the SSTable is within the parameters: `bucket_low </getting-started/compaction/#stcs-options>`_ and `bucket_high </getting-started/compaction/#stcs-options>`_, which is based on calculating the current average size of the SSTables already in the bucket. 
...
As noted in `this blog <http://www.datastax.com/dev/blog/when-to-use-leveled-compaction>`_, ``SizeTieredCompactionStrategy`` is great if partitions are written once and never modified (or written a few times and then not modified again). In that case, each partition will eventually end up being written as a whole to one compacted SSTable, and reads are efficient. However, when using STCS with workloads that continuously modifying existing pratitions, each partition is split across several SSTables, making reads slow. This doesn't happen with `Leveled compaction strategy (LCS)`_.
...
3. If after the compaction from Level 0 into Level 1, if there are more than 10 SSTables in Level 1, the excess SSTables from Level 1 are compacted and put into Level 2 as follows:
...
#. Take all of the output fragments and feed them back into compaction as a SSTable run.
#. Stop when all fragments from input runs were exhausted and released.

.. _note:
.. note:: To prevent data resurrection in case scylla crashes in the middle of compaction, ICS may possibly write an auxiliary run containing purgeable tombstones in addition to the output run containing live data. These are kept on disk while there are SSTables containing data they said tombstones may shadow. Once compaction is done deleting all shadowed data from all SSTables, the purgeable tombstones are purged and the SSTables holding them are removed from storage.
...
1. A time window is configured. The window is determined by the compaction window size `compaction_window_size </getting-started/compaction/#twcs-options>`_  and the time unit(`compaction_window_unit </getting-started/compaction/#twcs-options>`_). 
...
There can be cases where following a compaction tables are created in a level which after considerable amount of time is not compacted. For example, a user has tables which are using LCS. There are 5 levels of tables at present and the ``SSTable_size_in_mb`` is 5MB. The user changes this threshold to 160MB.  Following this change, there is only enough data to actually get a L3 on the same node. The data in the SSTables in L4 will get starved and will not get compacted. To avoid this, LCS tries to include those starved high level SSTables in future compactions. If after been 25 compaction rounds, a level was not compacted, it is brought in to the next compaction.
...
-----------------------------
Use the table in `Which strategy is best </architecture/compaction/compaction-strategies/>`_ to determine the right strategy for your needs. 
...
References
----------
* `CQL Reference for Compaction </getting-started/compaction/>`_
