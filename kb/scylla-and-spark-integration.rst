...
This is an example how to create a very simple Spark application that
...
With database containing all the necessary tables and data it is now
time to write our example application. Create directory
...
Now it is time to build the application and create self-containing jar
file that we will be able to send to Spark. To do that execute command:
...
Spark Web UI should be now available at http://localhost:8080. The Spark
...
them there should be message from the application:
...
These is a short guide explaining how to run a Spark example application
